...
#### 7.1 样本测试
- [ ] 7.1.1 准备 3-5 个不同岗位的画像 + JD + Gap 样本
- [ ] 7.1.2 逐样本验证：生成忠实度、Gap 改写映射体现、6 维评估合理性、PDF 排版
- [ ] 7.1.3 记录并修正 Prompt（生成 / 评估 / Gap 改写）

#### 7.2 边界与降级
- [ ] 7.2.1 边界：画像缺失、画像低质、无 Gap（通用降级）、迭代不收敛
- [ ] 7.2.2 降级：生成 LLM 失败、评估 LLM 失败（仅格式校验交付）、格式校验 ERROR 修复
- [ ] 7.2.3 路径 B：interrupt 恢复、画像追问、会话跨请求一致性
...
- [ ] 路径 B 精修闭环（interrupt → 反馈改写 → 画像追问 → 满意定稿）
- [ ] 简历按岗位多份、按版本多次持久化，可查历史与下载 PDF
- [ ] 前端完整（生成页、预览、评估报告、精修、历史）

### 质量标准
- [ ] 生成内容忠实于画像（不编造），量化可追溯
- [ ] Gap 四类改写策略在简历中可追溯体现
- [ ] 6 维评估稳定（同输入结果一致为主）
- [ ] 双层质量门（LLM 评估 + 格式校验）均生效
- [ ] 错误处理与降级完善

### 闭环标准
- [ ] 兑现 JD 匹配 design 决策 4 的"Gap → 简历优化"接口契约
- [ ] 路径 B 同构复用 mock-interview 的 interrupt/checkpointer
